...
| 一台跑wordpress mysql, 装web的虚拟机对外开放80端口, 以便能从公网访问。 用通常的手工方式可以做到，
| 即:
...
| 比较复杂有很多组件时，组件分布在多个虚拟机上时，手工的方式就不可接受了，这时我们就需要实现持续部署。
|       
2) Fit2Cloud的基本原理?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
|    5) 运行时环境安装配置脚本
|    6) 组件安装配置脚本
|    7) 集群力虚拟机启动或关闭时执行哪些脚本
|
|    Fit2Cloud会根据集群里的各个虚拟机组的定义或设置在指定的数据中心创建指定类型的虚拟机和执行指定的脚本。 
|
| 例子:
|   比如一个wordpress网站, 我们定义其为一个集群wordpress-dev，由两个虚拟机组组成
